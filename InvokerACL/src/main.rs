...
async fn main() -> CliResult {
    ...
    let function_path:String = env::var("FUNCTION_PATH").unwrap_or("C:\\Users\\Fabio Gentili\\Desktop\\InvokerACL\\function\\ACL-js-function\\accessControl.js".to_string());
    let trigger_topic = env::var("TRIGGER").unwrap_or("acl".to_string());
    let output_topic = env::var("OUTPUT").unwrap_or("output".to_string());
    let nats_server = env::var("NATSSERVER").unwrap_or("localhost:4222".to_string());
    ...
    for msg in sub.messages() {
        ...
        task::spawn( async move{
            ...
            if let Ok(payload) =  serde_json::from_str::<Payload>(&String::from_utf8_lossy(&msg.data)) {
                println!("received valid JSON payload: user={:?}, permission={:?}", payload.user, payload.permission);

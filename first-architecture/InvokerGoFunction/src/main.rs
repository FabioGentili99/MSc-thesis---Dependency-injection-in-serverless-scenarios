...
async fn main() -> CliResult {
    ...
    //Get ENV VAR
    let command: String = env::var("COMMAND").unwrap_or("".to_string());
    let function_path:String = env::var("FUNCTION_PATH").unwrap_or("../go/first-arch-go/functions/acl/handler_acl".to_string());
    ...
    for msg in sub.messages() {
        ...
        task::spawn( async move{
            ...
            //println!("{}", cm);
                ...
                let output = Command::new(&function_path)
                    //.arg(function_path)
                    //.arg("-c")
                    //.arg(&command)
                    .output()
